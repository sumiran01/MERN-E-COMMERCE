...
export const newOrder = Trycatch(async (req, res, next) => {
    const { shippingInfo, orderItems, tax, total, shippingCharges, subtotal, user, discount } = req.body;
...
export const MyOrder = Trycatch(async (req, res, next) => {
    ...
    if (mycache.has(`my-orders-${user}`))
        order = JSON.parse(mycache.get(`my-orders-${user}`));
    else {
...
export const allOrder = Trycatch(async (req, res, next) => {
    ...
    if (mycache.has(`all-orders`))
        order = JSON.parse(mycache.get(`all-orders`));
    else {
